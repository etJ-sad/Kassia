...
async def get_translation_api(language: str):
    ...
    try:
        ...
        if not translation_file.exists():
            ...
            # Return empty translations instead of 404 to prevent crashes
            return {}
        
        with open(translation_file, 'r', encoding='utf-8') as f:
            translations = json.load(f)
        
        logger.debug("Translation file served via API", LogCategory.API, {
            'language': language,
            'keys_count': len(translations)
        })
...
async def list_available_languages():
    ...
    try:
        ...
        logger.debug("Languages list requested", LogCategory.API, {
    ...
    try:
        ...
        debug_info = {
            "translations_directory": {
                ...
                "exists": translations_dir.exists(),
                "is_directory": translations_dir.is_dir() if translations_dir.exists() else False
            },
            "translation_files": [],
            "static_mount": {
                "mounted": True,  # We're using direct routes now
                "mount_path": "/static/translations"
            }
        }
            ...
            for json_file in translations_dir.glob("*.json"):
                try:
                    ...
                    file_info = {
                        ...
                        "keys_count": len(translations),
                        "valid_json": True,
                        "sample_keys": list(translations.keys())[:5] if translations else []
                    }
                except Exception as e:
                    file_info = {
                        "filename": json_file.name,
                ...
                debug_info["translation_files"].append(file_info)
        
        logger.info("Translation debug info requested", LogCategory.API, debug_info)
        return debug_info
...
async def test_translation(language: str, key: str):
    ...
    try:
        ...
        translation_value = translations.get(key)
        
        return {
            "language": language,
            "key": key,
            "found": key in translations,
